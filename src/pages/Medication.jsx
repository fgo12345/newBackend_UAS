...
const Medications = () => {
  ...
  const fetchMedications = async () => {
    ...
    try {
      const response = await fetch("https://uasbackend-production-3bed.up.railway.app/api/medications", {
  ...
  const toggleTaken = async (id, currentStatus) => {
    ...
    try {
      const response = await fetch(`https://uasbackend-production-3bed.up.railway.app/api/medications/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
